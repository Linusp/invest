...
组合接口以 `/portfolios` 为入口，包括组合属性 CRUD、期初状态、交易和持仓；原 `/strategies` 路径作为兼容接口保留。组合属性包括投资风格、是否自有资产、用途、投资方向、约束和备注；资金基准统一来自期初状态中的现金、历史净投入和历史已实现盈亏。组合页只常驻展示核心计算指标，其他属性从左侧组合列表的 info 入口查看。持仓、交易记录、组合点评和交易计划统一为四个列表 Tab，支持筛选、分页，以及表头“升序 → 降序 → 默认顺序”的三态排序。汇率接口为 `GET /exchange-rates/{currency}`。
